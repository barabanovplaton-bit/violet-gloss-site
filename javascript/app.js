...
// Other pages can await this to know if Firebase is available
window.vgFirebaseReady = new Promise(function(resolve) {
    window._vgFirebaseResolve = resolve;
    // Timeout: if Firebase doesn't init in 8 seconds, resolve with unavailable
    setTimeout(function() {
        if (!window.vgFirebase) {
            window.vgFirebase = { available: false };
            resolve(window.vgFirebase);
        }
    }, 8000);
});
